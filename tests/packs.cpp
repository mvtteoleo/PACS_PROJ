...
int main(int argc, char* argv[])
{
    ...
    std::cout << "Time above is for the normal for loop and " << Nx_dyn*Ny_dyn*Nz_dyn << "access\n";
    count = 0;
    ...
    std::cout << "Time above is for the all_element loopand " << Nx_dyn*Ny_dyn*Nz_dyn << "access\n";
    /*
    for (size_t i = 0; i < Nx_dyn; ++i)
        for (size_t j = 0; j < Ny_dyn; ++j)
            for (size_t k = 0; k < Nz_dyn; ++k)
