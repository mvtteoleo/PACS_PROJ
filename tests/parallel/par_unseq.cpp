...
int main(int argc, char** argv)
{
    ...
    tbb::global_control gc(tbb::global_control::max_allowed_parallelism, threads_per_rank);
    ...
    for (auto val : t)
    {
        if (val != 0.0)
        {
            unique_threads.insert(static_cast<size_t>(val));
        }
    }
