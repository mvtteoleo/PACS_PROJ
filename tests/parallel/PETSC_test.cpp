...
static inline PetscInt idx_from_ij(PetscInt i, PetscInt j, PetscInt nx) {
  return i + j * nx;
}

int main(int argc, char **args) {
  PetscErrorCode ierr;
  ierr = PetscInitialize(&argc, &args, (char*)0, "2D Laplacian example"); if (ierr) return ierr;
  ...
  ierr = PetscOptionsGetInt(NULL, NULL, "-ny", &ny, &flg); CHKERRQ(ierr);
  if (nx <= 0 || ny <= 0) SETERRQ(PETSC_COMM_SELF, PETSC_ERR_ARG_OUTOFRANGE, "nx, ny must be > 0");
  ...
  // Preallocation: up to 5 nonzeros per row (interior). Some boundary-adjacent rows will have fewer.
  ...
  for (PetscInt g = rstart; g < rend; ++g) {
    ...
    ierr = VecSetValue(x_true, g, uval, INSERT_VALUES); CHKERRQ(ierr);
    ...
    // -Delta u approximated by (2/hx^2 + 2/hy^2)*u_ij - (1/hx^2)*(u_{i-1}+u_{i+1}) - (1/hy^2)*(u_{j-1}+u_{j+1})
    ...
    ierr = MatSetValues(A, 1, &g, ncols, cols, vals, INSERT_VALUES); CHKERRQ(ierr);
  }

  // Finish assembling matrix and x_true
  ierr = MatAssemblyBegin(A, MAT_FINAL_ASSEMBLY); CHKERRQ(ierr);
  ierr = MatAssemblyEnd(A, MAT_FINAL_ASSEMBLY); CHKERRQ(ierr);
  ierr = VecAssemblyBegin(x_true); CHKERRQ(ierr);
  ierr = VecAssemblyEnd(x_true); CHKERRQ(ierr);
  ...
  ierr = VecAXPY(error, -1.0, x_true); CHKERRQ(ierr); // error = x - x_true
  PetscReal err_norm;
  ierr = VecNorm(error, NORM_2, &err_norm); CHKERRQ(ierr);
