...
void solve_neumann(int argc, char* argv[])
{
    ...
    if (!mpiRank) std::cout << "Initializing C2Decomp...\n";
    decomp.initialize_decomp(nx, ny, nz);
    if (!mpiRank) std::cout << "C2Decomp initialized.\n";
    ...
    if (!mpiRank)
    {
        std::cout << "Global max error = " << global_max_err << "\n";
...
void solve_dirich(int argc, char* argv[])
{
    ...
    auto Ly = ySizeArr[1]; // contiguous in Y-layout (jp) - see your indexing convention
    ...
    for (int kp = 0; kp < xSizeArr[2]; ++kp)
        for (int jp = 0; jp < xSizeArr[1]; ++jp)
        {
            std::copy_n(data1.ptr_at(1, jp, kp), Lx, xbuf);
            fftw_execute(fft_x);
            std::copy_n(xbuf, Lx, data1.ptr_at(1, jp, kp));
        }
...
int main(int argc, char* argv[])
{
    ...
    // dirich BC
#if NEUMANN == 0
    solve_dirich(argc, argv);
#elif NEUMANN == 1
    solve_neumann(argc, argv);
#endif
    return 0;
}
