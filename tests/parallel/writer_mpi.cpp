...
struct VTKStructuredWriter {
    ...
    void write(const Tensor& field, const std::string& base, double h) const {
        ...
        auto start = decomp.xStartWGhosts(); // Global starting point index (start_x, start_y, start_z)
        ...
        MPI_Gather(
            local_extent, 6, MPI_INT,
            all_extents.data(), 6, MPI_INT,
            0, MPI_COMM_WORLD
        );
    ...
    void write_pvts(const std::string& base, double h, int nproc, const std::vector<int>& all_extents) const {
        ...
        for (int r = 0; r < nproc; ++r) {
            ...
            ofs << "    <Piece Extent=\""
                ...
                << "Source=\"" << base_filename << "_" 
                << std::setw(4) << std::setfill('0') << r << ".vts\"/>\n";
        }
