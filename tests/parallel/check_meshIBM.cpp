...
int main(int argc, char* argv[])
{
    ...
    for (int k = 0; k < kMax; ++k)
        for (int j = 0; j < jMax; ++j)
            for (int i = 0; i < iMax; ++i)
            {
                ...
                // SCALAR BOXES
                is_scal_blocked.at(i, j, k, 0) = (check_pos.at(i, j, k, 0) == mask_v::inside)
                                                     ? status::blocked
                                                     : status::fluid_free;
    ...
    auto dz = static_cast<Real>(x_z - x_c[2]);

    Real r_p;
    if (dz < 0)
        r_p = 0;
    else
        r_p = std::sqrt(R2 - sqr(dz));
    ...
    for (size_t i = 0; i < iMax; ++i)
        for (size_t j = 0; j < jMax; ++j)
        {
            ...
            auto vp = is_scal_blocked.at(i, j, k_slice, 0);
            auto vu = is_scal_blocked.at(i, j, k_slice, 1);
            auto vv = is_scal_blocked.at(i, j, k_slice, 2);
    ...
    gp << "set title 'Mid-plane slice at " << k_slice << " '\n";
    gp << "set size square\n";
    ...
    gp << "plot "
       ...
       << x_c[0] << " + " << r_p << "*cos(t), " << x_c[1] << " + " << r_p << "*sin(t) "
       << "with lines lw 2 lc rgb 'blue' title 'Circle', "
       // Three datasets (p,u,v)
       << "'-' using 1:2:3 with points pt 2 ps 1.0 palette title 'p_pts', "
       << "'-' using 1:2:3 with points pt 7 ps 1.2 palette title 'u_pts', "
