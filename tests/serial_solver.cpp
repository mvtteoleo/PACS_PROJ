...
int main(int argc, char* argv[])
{
    ...
    std::size_t              N             = (argc > 1) ? std::stoul(argv[1]) : 100;
    ...
    numPDE::ScalarField<Real> h_P = P;
    numPDE::VectorField<Real> U(mesh);
    numPDE::VectorField<Real> h_U = U;
    ...
    for (int j = 0; j < N_TEST; ++j)
    {
        for (size_t i = 0; i < 2; ++i)
        {
            ...
            for (auto [i, j, k] : P.internal_elements())
            {
                ...
                numPDE::Vec<Real, 3> lap;
                lap = (E + W + N + S + T + B - 6.0 * C) / (h * h);

                numPDE::Vec<Real, 3> U_x, U_y, U_z, dU_dx, dU_dy, dU_dz;
            ...
            // SOLVE LAPLACE (INEFFICENTE AL MAX)
            // SOLVE LAPLACE PROBLEM
            // Compute step
            auto Jacobi_it = [&](auto idx)
            {
                auto [i, j, k] = idx;
                auto s_fij     = (U(i + 1, j, k)[0] - U(i - 1, j, k)[0]) / (2 * h) +
                             (U(i, j + 1, k)[1] - U(i, j - 1, k)[1]) / (2 * h) +
                             (U(i, j, k + 1)[2] - U(i, j, k - 1)[2]) / (2 * h);
                P(i, j) =
                    (h_P(i + 1, j, k) + h_P(i - 1, j, k) + h_P(i, j + 1, k) + h_P(i, j - 1, k) +
                     h_P(i, j + 1, k) + h_P(i, j - 1, k) + (h * h) * s_fij) /
                    6.;
            };
            auto check_stopping_crit = [&](auto idx)
            {
                auto [i, j, k] = idx;
                h_P(i, j, k)   = h_P(i, j, k) - P(i, j, k);
            };
            // while (toll < err)
            while (rips<3)
            {
                std::swap(P, h_P);
                P.for_intern(Jacobi_it);
                P.for_intern(check_stopping_crit);
                err = h_P.L2norm();
                /*
            ...
            for (auto [i, j, k] : P.internal_elements())
            {
                numPDE::Vec<Real> grad_P = {(P(i + 1, j, k) - P(i - 1, j, k)),
                                            (P(i, j + 1, k) - P(i, j - 1, k)),
                                            (P(i, j, k + 1) - P(i, j, k - 1))};
