...
namespace numPDE
{
    ...
    struct NSSolver
    {
        ...
        void pseudoTS(const type_solve &Buff, const type_solve &Un, const T dt_step, const T a=1.0)
        {
        ...
        void pseudoTS(const type_solve &Buff, const T dt_step, const T a=1.0)
        {
            ...
            for(const auto [k, j, i] : m_V.int_elems())
            {
                m_V(i, j, k) = m_V(i, j, k) + adt * Buff(i, j, k) - dt_step * (grad(m_P, i, j, k, h) /*+ r_inps.v_BC.f(pos)*/);
            }
        ...
        void apply_bc(T time) { if(!r_dec.rank()) printf("No BC impl yet"); }
    };
