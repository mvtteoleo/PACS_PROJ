...
auto fx(const T& x, const T& y, const T& z, const T& t, const T& Re=1.0) -> T
{
    ...
    return 3*std::pow(std::numbers::pi_v<T>, 2)*x6*x8/Re - 2*x10*x12*std::pow(std::sin(x2), 2) - x11*x12 + x11*std::pow(x7, 2) - x3*x7*x8*x9 + x6*std::cos(t);
}
template <typename T>
...
auto fz(const T& x, const T& y, const T& z, const T& t, const T& Re=1.0) -> T
{
    ...
    return 6*std::pow(std::numbers::pi_v<T>, 2)*x1*x10*x7/Re - x10*x11*x8*x9 + 4*x11*x12*x15 + x12*x13*x14*x16*std::pow(x9, 2) + x15*x16*std::pow(x8, 2) + x2*x7*std::cos(t);
}
}; //end numPDE