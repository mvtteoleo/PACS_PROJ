...
namespace numPDE
{
    ...
    /* Re=1.0 even thos is not needed for the velocity cause I wanted to keep the least changes
     * possible */
    template <typename T>
    auto ux(const T& x, const T& y, const T& z, const T& t, const T& Re = 1.0) -> T
    {
    ...
    template <typename T>
    auto uy(const T& x, const T& y, const T& z, const T& t, const T& Re = 1.0) -> T
    {
    ...
    template <typename T>
    auto uz(const T& x, const T& y, const T& z, const T& t, const T& Re = 1.0) -> T
    {
    ...
    auto fx(const T& x, const T& y, const T& z, const T& t, const T& Re = 1.0) -> T
    {
        ...
        return 3 * std::pow(std::numbers::pi_v<T>, 2) * x10 * x3 * x5 * x9 / Re -
               2 * std::pow(x1, 2) * x11 * x13 + x1 * x7 - x12 * x13 +
               x12 * std::pow(std::cos(x2), 2) + x7 * x9 * std::cos(x8);
    }
    template <typename T>
    ...
    auto fz(const T& x, const T& y, const T& z, const T& t, const T& Re = 1.0) -> T
    {
        ...
        return 6 * std::pow(std::numbers::pi_v<T>, 2) * x10 * x6 * x8 / Re +
               std::numbers::pi_v<T> * x6 * x9 * std::cos(x7) + 4 * x1 * x11 * x12 * x13 * x8 +
               x1 * x6 + x12 * x14 * std::pow(std::cos(x2), 2) +
               x13 * x14 * std::pow(std::cos(x4), 2);
    }
}; // namespace numPDE