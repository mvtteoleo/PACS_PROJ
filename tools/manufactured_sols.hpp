...
namespace numPDE
{
    ...
    {
        return std::cos(std::numbers::pi_v<T> * x) * std::cos(std::numbers::pi_v<T> * y) *
               std::cos(std::numbers::pi_v<T> * z);
    }
    template <typename T>
    auto fx(const T& x, const T& y, const T& z, const T& t, const T& Re = 1.0) -> T
    {
        ...
        return 3 * std::pow(std::numbers::pi_v<T>, 2) * x10 * x11 * x5 / Re +
               std::pow(x1, 2) * x13 - x1 * x3 * x6 + x10 * x6 * std::cos(x7) -
               2 * x12 * x14 * std::pow(std::sin(x4), 2) - x13 * x14;
    }
    template <typename T>
    ...
    auto fz(const T& x, const T& y, const T& z, const T& t, const T& Re = 1.0) -> T
    {
        ...
        return 6 * std::pow(std::numbers::pi_v<T>, 2) * x11 * x12 * x5 / Re +
               std::pow(x1, 2) * x14 * x17 - x1 * x3 * x6 + x11 * x8 * std::cos(x7) +
               4 * x13 * x14 * x15 * x16 * x6 + x16 * x17 * std::pow(x3, 2);
    }
}; // namespace numPDE